        ...
        <View style={styles.body}>
            ...
            <View style={styles.table}>
                ...
                {result?.data?.map((item, index) => (
                    <TouchableOpacity key={item._id} style={styles.row} onPress={() => onReview(item._id, item.examination._id)}>
                        <Text style={[styles.text, { width: '15%' }]}> {index + 1}</Text>
                        <Text style={[styles.text, { width: '45%' }]}>{item.examination.name}</Text>
                        <Text style={[styles.text, { width: '25%' }]}>{item.examination.type}</Text>
